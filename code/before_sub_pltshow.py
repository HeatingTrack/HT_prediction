...
def draw_graph(k, year):
    ...
    for i, one_mon in enumerate(year):
        data = pd.read_csv('C:/Users/user/Desktop/Project/Gas/data/filtered/' + k + '/' + k + '_' + str(one_mon))
        x_values = range(100)  # 처음 100개 데이터만 사용
        y_values = data[selected_column].values[:100]  # 처음 100개 데이터에 해당하는 y 값
        row = i // 4
        col = i % 4
        ax = axs[row, col]
