...
for mon in combined_data_list:
    data = pd.read_csv('C:/Users/user/Desktop/Project/Gas/data/filtered/oneroom/oneroom_' + str(mon), encoding="utf-8")
...
# Multiple Linear Regression 모델 설정
...
print(f'Multiple Linear Regression 모델 - Root Mean Squared Error (MSE): {mse}')
...
plt.xlabel('데이터 포인트')
plt.ylabel('USEAMT')
plt.title('실제 데이터 분포 및 Linear Regression 모델 예측')
plt.legend()
plt.show()